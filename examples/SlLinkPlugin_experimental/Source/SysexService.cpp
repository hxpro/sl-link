...
SysexService::SysexService(SlLinkPluginAudioProcessor& proc)
: mainProc(proc)/*, audioSetupComp(audioDeviceManager, 0, 0, 0, 0, true, true, false, false)*/
{
    deviceID = ++DEVICE_ID;
    deviceNAME = MY_DEVICE_NAME + String("_") + String(deviceID);
...
//        if(mOut) mOut->sendMessageNow(sysMsg);
        ...
        sysMsg.setTimeStamp(Time::getMillisecondCounterHiRes() * 0.001); // tempo in secondi
        midiCollector.addMessageToQueue(sysMsg);
    }
}

void SysexService::sendIdentity()
{
    ...
    String text = deviceNAME;// MY_DEVICE_NAME;
...
void SysexService::sendMidiMessage(const MidiMessage& sysMsg)
{
    queueOut.push_back(sysMsg);
}

void SysexService::sendIcon(uint8_t packet_num)
{
    // Start with the manufacturer ID
    std::vector<uint8_t> sysexMessage {
        FATAR_SYSEX_ID[0],
...
void SysexService::logoutRequest()
{
    // Start with the manufacturer ID
    std::vector<uint8_t> sysexMessage {
        FATAR_SYSEX_ID[0],
...
//void SysexService::handleIncomingMidiMessage(MidiInput* source, const MidiMessage& message)
//{
//    // Il MIDI input, come l'output, è gestito da una coda temporizzata
//    queueIn.push_back(message);
//}

void SysexService::timerCallback(int timerID)
{
    switch (timerID)
    {
        case TIMER_IDENTITY:
...
void SysexService::parseMidiInput(const MidiMessage& message)
{
    ...
    uint8_t devId = message.getSysExData()[5];
...
void SysexService::handleSystemMessage(uint8_t item_num, uint8_t item_val)
{
    switch (item_num)
    {
...
void SysexService::handleButtonMessage(uint8_t item_num, uint8_t item_val)
{
    switch (item_num)
    {
...
void SysexService::handleEncoderMessage(uint8_t item_num, uint8_t item_val)
{
...
void SysexService::LED_RGB(uint8_t num, uint8_t r, uint8_t g, uint8_t b, uint8_t brigh)
{
...
void SysexService::LED_WHITE(uint8_t num, uint8_t state)
{
    // Start with the manufacturer ID
    std::vector<uint8_t> sysexMessage {
        FATAR_SYSEX_ID[0],
...
void SysexService::PLOT_TEXT(String text,
                               uint16_t x,
                               uint16_t y,
                               uint16_t width,
                               uint8_t txIndent,
...
void SysexService::PLOT_RECT(uint16_t x,
                               uint16_t y,
                               uint16_t w,
                               uint16_t h,
...
void SysexService::PLOT_BITMAP(uint16_t x,
                                 uint16_t y,
                                 uint8_t bmp_gdx,
                                 uint8_t bmp_idx,
                                 uint8_t r_fore, uint8_t g_fore, uint8_t b_fore,
...
void SysexService::CLEAR_LCD(uint8_t r, uint8_t g, uint8_t b)
{
    // Convert from 8 8 8 to 7 7 7
    r = r >> 1;
    g = g >> 1;
...
void SysexService::drawScreen()
{
    CLEAR_LCD();

    PLOT_RECT(0, 0, LCD_WIDTH, 100,
        155, 127, 64);
    PLOT_TEXT(deviceNAME, 10, 40, 300,
              TEXT_ALIGN_CENTER,
              TEXT_SIZE_MIDDLE,
              0, 0, 0,
              155, 127, 64);
